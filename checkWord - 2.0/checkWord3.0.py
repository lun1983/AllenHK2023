...
# 20231016: V3.0 将目录率先扫描出来，放到resut的头上
"""
...
def checkEachFile(CHECK_FILE, READ_FILE):
    ...
    sec_cnt = 0
    tot_sec_cnt = 0
    CHECK_RESULT.append('------章條 START-----')
    for item in READ_FILE:
        if '第' in item and '章' in item and item.lstrip().startswith('第'):
            CHECK_RESULT.append(' <<< ' + str(item.lstrip()).split('章')[0] + '章 >>>')
            sec_cnt = 0
        if '第' in item and '條' in item and item.lstrip().startswith('第'):
            sec_cnt += 1
            tot_sec_cnt += 1
            CHECK_RESULT.append(str(sec_cnt) + ':' + str(item.lstrip()).split('條')[0] + '條')

    CHECK_RESULT.append('-----章條 END-----' +' TOTAL: ' +str(tot_sec_cnt) +'條')
